...
app.get("/jokes/:id", (req, res) => {
  ...
  console.log(id);
  const result = jokes.find((joke) => joke.id === Number.parseInt(id));
...
app.get("/filter", (req, res) => {
  ...
  console.log(type);
  const result = jokes.filter((joke) => joke.jokeType.toLowerCase() === type);
  if (!result) return res.sendStatus(400);
  return res.status(200).send(result);
});
...
app.post("/jokes", (req, res) => {
  ...
  if (!newJoke.jokeText || !newJoke.jokeType) return sendStatus(400); //bad request

  const foundJoke = jokes.find((joke) => joke.jokeText === newJoke.jokeText);
  ...
  return res.status(409).send({ message: "Joke ALready Exists!!!" });
  // 409 -Conflict
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  console.log(jokeIndex ? true : false);
  ...
  if (!replacementJoke.jokeText || !replacementJoke.jokeType)
    return res.sendStatus(400);

  if (!jokeIndex) return res.status(404).send({ message: "Joke Not Found!!" });
